...
const BASE_URL = "https://www.pildorasinformaticas.es/all-courses/";
...
const scrapping = async () => {
  ...
  await page.waitForTimeout(4000); //le decimos al navegador que se espere 4seg para cargar bien toda la info
  ...
  const tittle = await page.$$eval(".item-title", (nodes) =>
  ...
  const image = await page.$$eval(
    ...
    (nodes) => nodes.map((n) => n.srcset)
  );
  const price = await page.$$eval(".item-credits", (nodes) =>
    nodes.map((n) => n.innerText)
  );
  const description = await page.$$eval(".item-desc", (nodes) => {
    nodes.map((n) => n.innerText);
  });
  console.log(tittle);
  ...
  fs.writeFile("cursos.json", stProducts, () => {
    console.log("copiado");
  });
